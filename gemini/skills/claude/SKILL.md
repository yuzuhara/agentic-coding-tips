...
description: |
  ...
  トリガー: "claude", "claudeと相談", "claudeに聞いて", "Claudeの意見", "Anthropicに聞いて", "セカンドオピニオン", "別の意見"
  使用場面: (1) 文言・メッセージの検討、(2) コードレビュー、(3) 設計の相談、(4) バグ調査、(5) 別のAIからのセカンドオピニオンが欲しいとき
---

# Claude CLI 呼び出しスキル

Gemini作業中に、Claude CLI（Anthropic）を呼び出してセカンドオピニオンや分析を得るためのスキル。
...
### Gemini結果のクロスチェック
```bash
claude -p --model sonnet "以下のGeminiによるコードレビュー結果を精査し、追加の指摘や異なる見解があれば提示してください。
